...
require 'C:\wamp64\www\fameink\front\phpmailer\vendor\autoload.php';
    ...
    try {
		...
		$mail->isHTML(TRUE);
        $mail->Body = "<html><h1 style='background-color:gold;color:gray;text-align:center'>ACTIVEZ VOTRE COMPTE</h1><p style='text-align:center;color:white;background-color:slategray;'>Cliquez sur le lien suivant pour activer votre compte : </p><a href='http://localhost/fameink/front/connexion/inscription/inscription.php?nom=".$_SESSION['nom']."&amp;prenom=".$_SESSION['prenom']."&amp;datenaiss=".$_SESSION['datenaiss']."&amp;telephone=".$_SESSION['telephone']."&amp;email=".$_SESSION['email']."&amp;password=".$_SESSION['password']."&amp;pseudo=".$_SESSION['pseudo']."&amp;pays=".$_SESSION['pays']."&amp;ville=".$_SESSION['ville']."&amp;quartier=".$_SESSION['quartier']."&amp;rue=".$_SESSION['rue']."&amp;poste=".$_SESSION['poste']."&amp;plus=".$_SESSION['plus']."' style='color:green;text-align:center;'>j'active mon compte</a></html>";
        $mail->isSMTP();
        $mail->Port = 587;
        $mail->SMTPAuth = TRUE;
        $mail->SMTPSecure = 'tls';
