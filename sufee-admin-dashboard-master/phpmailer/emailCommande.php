...
require 'C:\wamp64\www\sufee-admin-dashboard-master\phpmailer\vendor\autoload.php';
    ...
    try {
        ...
        $mail->setFrom($google_email, 'Projet');
        $mail->addAddress($user["email"], $user['nom']);
        $mail->Subject = 'COMMANDE VALIDEE';
		$mail->isHTML(TRUE);
        $mail->Body = "<html><h1 style='background-color:gold;color:green;text-align:center'>COMMANDE VALIDEE</h1><p style='text-align:center;color:white;background-color:slategray;'>M/Mlle '".$user['nom']."' votre commande a été validée avec succès. Nous vous tiendrons informé de la suite </p></html>";
        $mail->isSMTP();
        $mail->Port = 587;
        $mail->SMTPAuth = TRUE;
        $mail->SMTPSecure = 'tls';
